...
µmol$\cdot$m<sup>-2</sup>$\cdot$s<sup>-1</sup>) and $t$ is in seconds.
Therefore, the units of kappa-max is s<sup>-1</sup>. This limit of
kappa-max is included in the `goFlux()` function, so that the non-linear
flux estimate cannot exceed this maximum curvature. See below for more
...
- [**G-factor**](#g-factor): the ratio between a non-linear flux
  estimate (e.g. Hutchinson and Mosier; HM) and a linear flux estimate.
- [**Kappa ratio**](#kappa-ratio): maximal limit of the ratio between
  kappa and kappa-max ($k.max$).
