...
## Function Documentation Validation {#sec-validate-plots}

This section automatically validates that the plotting functions are properly documented:
...
if (requireNamespace("goFlux", quietly = TRUE)) {
  cat("📊 **Plotting Functions in goFlux Package:**\n\n")
  
  # flux.plot arguments
  fluxplot_args <- formals(goFlux::flux.plot) 
  cat("**flux.plot arguments:**\n")
  for (i in seq_along(fluxplot_args)) {
    arg_name <- names(fluxplot_args)[i]
    # Safely deparse and collapse in case result is multi-line
    arg_str <- paste(deparse(fluxplot_args[[i]]), collapse = " ")
  ...
  cat("\n**flux2pdf arguments:**\n")
  flux2pdf_args <- formals(goFlux::flux2pdf)
  for (i in seq_along(flux2pdf_args)) {
    arg_name <- names(flux2pdf_args)[i]
    # Safely deparse and collapse in case result is multi-line
    arg_str <- paste(deparse(flux2pdf_args[[i]]), collapse = " ")
