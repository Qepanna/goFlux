...
if (requireNamespace("goFlux", quietly = TRUE)) {
  ...
  for (arg_name in names(fluxplot_args)) {
    default_val <- fluxplot_args[[arg_name]]
    if (is.null(default_val)) {
      ...
      cat(sprintf("- `%s` = c(%s)\n", arg_name, paste(deparse(default_val), collapse = "")))
    } else {
  ...
  for (arg_name in names(flux2pdf_args)) {
    default_val <- flux2pdf_args[[arg_name]]
    if (is.null(default_val)) {
      cat(sprintf("- `%s` (required)\n", arg_name))
    } else if (is.character(default_val)) {
      cat(sprintf("- `%s` = \"%s\"\n", arg_name, default_val))
