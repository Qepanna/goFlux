...
### `best.flux` for automatic selection of best flux estimate
...
| `criteria` | character vector; criteria used to assess the goodness of fit of the LM or HM flux estimates. Must be at least one the following: "MAE", "RMSE", "AICc", "SE", "g.factor", "kappa... |
| `intercept.lim` | numerical vector of length 2; inferior and superior limits of the intercept (initial concentration; html C<sub>0</sub> C[0] ASCII ). Must be the same units as gastype in the goF... |
| `g.limit` | numerical value; maximum limit of the g-factor (ratio between HM.flux and LM.flux ). Recommended thresholds for the g-factor are 4 (flexible), 2 (medium), or 1.25 (conservative)... |
...
In , the indices of model fit "MAE", "RMSE" and "SE" all have a threshold. For MAE and RMSE, the threshold is instrument precision (1 , ) ). For SE, the threshold is the instrument accuracy (1 , ) / , ) ). These indices also compare the two models (linear, LM, and non-linear, HM). The selection of the best model based on indices of model fit ("MAE", "RMSE", "AICc" and "SE") is based on a scoring system. Both models start with a score of 0. For each criteria, whichever model performs worst is given +1. After all selected criteria have been evaluated, the model with the lowest score wins. In case of a tie, the HM flux estimate is always chosen by default, as non-linearity is assumed. The score is printed in the columns HM.score and LM.score. The indicates a threshold for the ) , which is the ratio between a non-linear flux estimate and a linear flux estimate. With the ) function, one can choose a limit at which the HM model is deemed to overestimate , ), , ))) . Recommended thresholds for the g-factor are <4 for a flexible threshold, <2 for a medium threshold, or <1.25 for a more conservative threshold. The default threshold is . If the g-factor is above the specified threshold, the best flux estimate will switch to LM instead of HM and give a warning in the columns and . The minimal detectable flux ( ) ) is calculated from the instrument precision, the measurement time and the flux.term. Flux estimates below the MDF are considered under the detection limit, but not null. Therefore, the function will not return a 0. There will simply be a warning in the columns , or to warn of a flux estimate under the detection limit. No best flux estimate is chosen based on MDF. The parameter kappa determines the curvature of the non-linear regression in the Hutchinson and Mosier model. A maximum value of kappa, ) is included in the ) function, so that the HM regression cannot exceed this maximum curvature. In the function ) , one can choose the linear flux estimate over the HM flux estimate based on the ratio between kappa and kappa-max ( ). The ratio is expressed as a percentage, where 1 indicates , and 0.5 indicates . The default setting is . If is larger than , a warning is issued in the columns and . If the initial gas concentration ( , ), , ))) ) calculated for the flux estimates ( and ) deviates from the true ( , ), , ))) ) by more or less than 10% of the difference between , ), , ))) and the final gas concentration at the end of the measurement ( , ), , ))) ), a warning is issued in the columns , or to warn of an intercept out of bounds. Alternatively, one can provide boundaries for the intercept, for example: ") for a true , ), , ))) of 400 ppm. The argument is only applicable to the linear flux. Under the defined , the slope of the linear flux estimate is deemed non-significant, i. e., flux under the detection limit. The default threshold is . No best flux estimate is chosen based on . If , a warning is given in the columns and to warn of an estimated flux below the detection limit. is the limit below which the chamber closure time is flagged for being too short ( ). Portable greenhouse gas analyzers typically measure at a frequency of 1 Hz. Therefore, for the default setting of , the chamber closure time should be approximately one minute (60 seconds). If the number of observations is smaller than the threshold, a warning is issued in the column .
