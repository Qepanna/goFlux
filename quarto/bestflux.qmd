...
## Function Documentation Validation {#sec-validate-bestflux}

This section automatically validates that the documentation above matches the actual `best.flux` function:
...
if (requireNamespace("goFlux", quietly = TRUE)) {
  bestflux_args <- formals(goFlux::best.flux)
  
  cat("📋 **Current best.flux Function Arguments:**\n\n")
  for (i in seq_along(bestflux_args)) {
    arg_name <- names(bestflux_args)[i]
    # Safely deparse and collapse in case result is multi-line
    arg_str <- paste(deparse(bestflux_args[[i]]), collapse = " ")
