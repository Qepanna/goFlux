...
## Function Documentation Validation {#sec-validate-goflux}

This section automatically validates that the documentation above matches the actual `goFlux` function:
...
if (requireNamespace("goFlux", quietly = TRUE)) {
  goflux_args <- formals(goFlux::goFlux)
  
  cat("📋 **Current goFlux Function Arguments:**\n\n")
  for (i in seq_along(goflux_args)) {
    arg_name <- names(goflux_args)[i]
    # Safely deparse and collapse in case result is multi-line
    arg_str <- paste(deparse(goflux_args[[i]]), collapse = " ")
    
    # Check if argument has no default (missing/empty)
    if (arg_str == "" || arg_str == "NULL") {
      cat(sprintf("- `%s` (required)\n", arg_name))
    } else if (nchar(arg_str) > 60) {
      # Complex default, show truncated version
      cat(sprintf("- `%s` = %s\n", arg_name, substring(arg_str, 1, 50)))
    } else {
      cat(sprintf("- `%s` = %s\n", arg_name, arg_str))
    }
  }
  ...
  cat(sprintf("\n**Total: %d arguments**\n", length(goflux_args)))
  cat("\n✅ This documentation is automatically synchronized with the R package.")
