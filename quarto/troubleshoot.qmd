...
3.  **Update other required packages**. Some errors may occur due to outdated packages. When installing the package `goFlux` for the first time, it will ask you if you want to update the already installed dependencies. You should always choose to update these packages. If you are using RStudio, there is a chance that the program fails to install some of the packages because it is not allowed to delete the previous version. To solve this issue, manually delete the packages you want to update (click on the little x in the "Packages" tab) before re-installing them.
