...
#' file.path <- system.file("extdata", "LI7810/example_LI7810.data", package = "GoFluxYourself")
#'
#' LI7810.data <- LI7810_import(inputfile = file.path)
#'
#' @export
#'
LI7810_import <- function(inputfile, date.format = "ymd",
                          timezone = "UTC", save = FALSE){

  # Assign NULL to variables without binding
  H2O_ppm <- H2O <- CH4 <- CO2 <- TIME <- DATE <- DATAH <- NULL
  ...
  data.raw <- read.delim(inputfile, skip = skip.rows) %>%
    ...
    select(DATE, TIME, CO2dry_ppm = CO2, CH4dry_ppb = CH4, H2O_ppm = H2O) %>%
  ...
  if(save == TRUE){
    ...
    outputfile <- paste("LI7810_", sub("\\.data", "", inputfile), "_imp.Rdata", sep = "")
