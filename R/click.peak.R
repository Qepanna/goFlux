...
click.peak <- function(flux.unique, gastype = "CO2dry_ppm", sleep = 3,
  ...
  yaxis.lim.max <- ifelse(ymax+0.05*ymax > yaxis.limit.max, yaxis.limit.max, ymax+0.05*ymax)
  yaxis.lim.min <- ifelse(ymin-0.05*ymax < yaxis.limit.min, yaxis.limit.min, ymin-0.05*ymax)
